...
type systemSettingsResponse struct {
	...
	// ForbidOrgWideServiceAccounts: compliance posture — new service
	// accounts must be scoped (visibility via group memberships); the
	// org_wide scope is rejected, and existing org-wide SAs resolve as
	// scoped while enabled (docs/service-account-scoping-design.md).
	ForbidOrgWideServiceAccounts bool `json:"forbid_org_wide_service_accounts"`
}
...
type systemSettingsRequest struct {
	...
	ForbidOrgWideServiceAccounts *bool   `json:"forbid_org_wide_service_accounts,omitempty"`
}
...
func (h *Handlers) systemSettings(r *http.Request) (systemSettingsResponse, error) {
	...
	forbidOrgWideSAs, err := h.Settings.GetForbidOrgWideServiceAccounts(r.Context())
	if err != nil {
		return systemSettingsResponse{}, err
	}
	resp := systemSettingsResponse{Environment: env, IngestBaseURL: ingest, IngestURLSource: "setting", MapHTTP5xxToError: map5xx, ForbidOrgWideServiceAccounts: forbidOrgWideSAs}
...
func (h *Handlers) patchSystemSettings(w http.ResponseWriter, r *http.Request) {
	...
	if body.Environment == nil && body.IngestBaseURL == nil && body.MapHTTP5xxToError == nil && body.ForbidOrgWideServiceAccounts == nil {
	...
	if body.ForbidOrgWideServiceAccounts != nil {
		if err := h.Settings.SetForbidOrgWideServiceAccounts(r.Context(), *body.ForbidOrgWideServiceAccounts); err != nil {
			h.Logger.Error("set forbid_org_wide_service_accounts failed", "err", err)
			httpserver.WriteError(w, http.StatusInternalServerError, "save failed")
			return
		}
		audit["forbid_org_wide_service_accounts"] = *body.ForbidOrgWideServiceAccounts
	}
