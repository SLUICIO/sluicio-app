...
type systemCheck struct {
	...
	SplitBy string
	// Scope declares what the check is ABOUT: alerting.CheckScopeProcess
	// (the service as a process, so true of every flow on it) or
	// alerting.CheckScopeFlow (rows belonging to one flow). Empty leaves
	// it inferred.
	//
	// It decides whether an integration holding a SLICE of the service
	// reads the check as its own, which is why the shared-runtime types
	// declare theirs: the inference cannot tell a filter that selects a
	// flow from one that selects a series of the same process.
	Scope string
...
var monitoringTemplates = []monitoringTemplate{
	...
	},
	{
		...
		Checks: []systemCheck{
			{Name: "DAG task failures", Description: "Task instances failed, split by DAG so the firing names which one. This is the per-DAG failure signal; bind it to an integration per DAG to alert on one flow alone.", Metric: "airflow.ti_failures", Agg: alerting.AggIncrease, Op: alerting.OpGT, Threshold: 0, SplitBy: "dag_id", Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning, Unit: "tasks", Display: true},
			{Name: "Operator failures", Description: "Failures counted per operator as well as per DAG - the same events as above seen by the kind of task that raised them, which is what tells a flaky sensor from a broken load step.", Metric: "airflow.operator_failures", Agg: alerting.AggIncrease, Op: alerting.OpGT, Threshold: 0, SplitBy: "dag_id", Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning, Unit: "tasks"},
			{Name: "Scheduler not running", Description: "The scheduler heartbeat stopped. Fires on the ABSENCE of the metric, because a scheduler that dies emits nothing left to threshold - which is why nothing else here would notice. Every DAG is stopped while this holds.", Metric: "airflow.scheduler_heartbeat", Agg: alerting.AggIncrease, Op: alerting.OpLT, Threshold: 1, FireOnNoData: true, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityCritical, Display: true},
			{Name: "Tasks starving for a pool slot", Description: "Tasks are runnable but have no slot, so they wait without failing - a queue nobody is told about. Left unsplit on purpose: it affects every DAG drawing on that pool, and splitting by pool_name would stop it saying so.", Metric: "airflow.pool.starving_tasks", Agg: alerting.AggMax, Op: alerting.OpGT, Threshold: 0, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityWarning, Unit: "tasks", Display: true},
			{Name: "Executor out of slots", Description: "No open executor slots, so nothing new starts however much is queued. Raise the threshold above zero to fire before saturation rather than at it.", Metric: "airflow.executor.open_slots", Agg: alerting.AggMin, Op: alerting.OpLT, Threshold: 1, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityWarning, Unit: "slots", Display: true},
			{Name: "Tasks queued and not starting", Description: "The queue is deep. Normal during a burst, and a stuck executor otherwise - tune to what your executor clears in five minutes.", Metric: "airflow.executor.queued_tasks", Agg: alerting.AggMax, Op: alerting.OpGT, Threshold: 50, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityWarning, Unit: "tasks"},
		},
	},
	{
		...
		Checks: []systemCheck{
			{Name: "DAG processor not running", Description: "The heartbeat stopped, so no DAG file is being reparsed: edits never take effect and a new DAG never appears. Fires on the absence of the metric, because a dead process emits nothing left to threshold.", Metric: "airflow.dag_processor_heartbeat", Agg: alerting.AggIncrease, Op: alerting.OpLT, Threshold: 1, FireOnNoData: true, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityCritical, Display: true},
			{Name: "DAG import errors", Description: "A DAG file fails to import. That DAG is not scheduled at all, and it fails nowhere: no task errors, no failed run, nothing on the scheduler. Scheduled work vanishes quietly.", Metric: "airflow.dag_processing.import_errors", Agg: alerting.AggMax, Op: alerting.OpGT, Threshold: 0, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityCritical, Unit: "files", Display: true},
			{Name: "No DAGs loaded", Description: "The DAG bag is empty - a bundle that failed to sync, or a path that moved. Raise the threshold to your DAG count to catch losing SOME of them rather than all.", Metric: "airflow.dagbag_size", Agg: alerting.AggMin, Op: alerting.OpLT, Threshold: 1, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityCritical, Unit: "dags"},
			{Name: "DAG file not reparsed", Description: "A file has not been reparsed recently, so edits to it are not live. The value is seconds since its last parse, split by file so the firing names it.", Metric: "airflow.dag_processing.last_run.seconds_ago", Agg: alerting.AggMax, Op: alerting.OpGT, Threshold: 600, SplitBy: "file_name", Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning, Unit: "seconds"},
			{Name: "DAG parsing slow", Description: "Total parse time is high, which delays every schedule. Usually one file doing real work at import time - an API call or a database query at module level. Tune to your DAG count.", Metric: "airflow.dag_processing.total_parse_time", Agg: alerting.AggMax, Op: alerting.OpGT, Threshold: 30, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityWarning, Unit: "seconds", Display: true},
		},
	},
	{
		...
		Checks: []systemCheck{
			{Name: "Triggerer not running", Description: "The heartbeat stopped. Every deferred task stays deferred: not running, not failing, indistinguishable from waiting correctly. Fires on the absence of the metric.", Metric: "airflow.triggerer_heartbeat", Agg: alerting.AggIncrease, Op: alerting.OpLT, Threshold: 1, FireOnNoData: true, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityCritical, Display: true},
			{Name: "Trigger blocking the main thread", Description: "A trigger blocked the async loop. One badly written trigger doing synchronous work starves every other deferred task on this process, so the symptom is unrelated tasks hanging.", Metric: "airflow.triggers.blocked_main_thread", Agg: alerting.AggIncrease, Op: alerting.OpGT, Threshold: 0, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityWarning, Unit: "triggers", Display: true},
			{Name: "Triggerer at capacity", Description: "No capacity left for new deferred tasks, so they queue behind the ones running. Scale the triggerer or raise its capacity.", Metric: "airflow.triggerer.capacity_left", Agg: alerting.AggMin, Op: alerting.OpLT, Threshold: 1, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityWarning, Unit: "slots", Display: true},
		},
	},
	{
	...
	},
	{
		...
		Checks: []systemCheck{
			{Name: "Route failing", Description: "Exchanges failed on a route without being handled by an error handler. The split names which route, not just which JVM.", Metric: "camel.exchanges.failed", Agg: alerting.AggIncrease, Op: alerting.OpGT, Threshold: 0, SplitBy: "routeId", Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning, Unit: "exchanges", Display: true},
			{Name: "Route dead-lettering", Description: "Failures are being caught and handled - a dead-letter channel doing its job, which is a silent failure if nobody reads that queue. Raise or disable this if dead-lettering is your designed path.", Metric: "camel.exchanges.failures.handled", Agg: alerting.AggIncrease, Op: alerting.OpGT, Threshold: 10, SplitBy: "routeId", Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning, Unit: "exchanges"},
			{Name: "Route backlog", Description: "Exchanges are piling up in flight on a route - it is wedged, or intake is outrunning it. Tune to the concurrency the route is built for.", Metric: "camel.exchanges.inflight", Agg: alerting.AggMax, Op: alerting.OpGT, Threshold: 100, SplitBy: "routeId", Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning, Unit: "exchanges", Display: true},
			{Name: "Routes stopped", Description: "Running routes dropped to zero. Raise the threshold to your route count so losing ONE route fires, rather than only the context going down entirely.", Metric: "camel.routes.running", Agg: alerting.AggMin, Op: alerting.OpLT, Threshold: 1, Scope: alerting.CheckScopeProcess, Severity: alerting.SeverityCritical, Unit: "routes", Display: true},
			{Name: "Broker redelivering to a route", Description: "An external broker is redelivering messages to this route - the consumer keeps rejecting them, which ends in a dead-letter queue rather than a visible error.", Metric: "camel.exchanges.external.redeliveries", Agg: alerting.AggIncrease, Op: alerting.OpGT, Threshold: 0, SplitBy: "routeId", Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning, Unit: "redeliveries"},
			{Name: "Exchange failed", Description: "A failed exchange, from the trace rather than the counter: it names the endpoint and the step, which the metric cannot. Needs camel-opentelemetry.", Signal: "trace_error", TraceThreshold: 1, WindowSeconds: 900, Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning},
			{Name: "Slow exchanges", Description: "p95 exchange duration is high. Tune to the route's own shape - a file poll and an HTTP call do not belong to the same threshold.", Signal: "trace_latency", ThresholdMs: 30000, WindowSeconds: 900, Scope: alerting.CheckScopeFlow, Severity: alerting.SeverityWarning, Unit: "ms"},
		},
	},
	{
