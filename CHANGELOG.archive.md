# Changelog archive — pre-public history

_The sections below predate the public GitHub repository. The public repo
starts from a fresh root commit at v0.11.1, so the commit SHAs referenced
here do not resolve there; the full development history lives on the
private mirror (branch `pre-public-history`)._
