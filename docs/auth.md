...
### "A platform runs Sluicio for us" (managed mode)

An instance can also be run by a platform on behalf of the people using
it - a hosting provider, a partner, an internal team running instances
for other departments. That instance is reachable before its owner has
ever opened it, which makes the out-of-the-box story above unsafe: a
documented `admin`/`admin` on a public address belongs to whoever finds
it first.

`SLUICIO_MANAGED=true` switches that around:

  - **The seeded admin gets no password.** There is nothing to sign in
    with, so there is nothing to guess.
  - **First-user setup needs a token**, `SLUICIO_BOOTSTRAP_TOKEN`:
    random, one per instance, generated by whatever deploys it. The owner
    receives it as a setup link whose fragment carries it,
    `https://<instance>/#t=<token>`. A fragment never reaches the server,
    so the token stays out of access logs and `Referer` headers; the page
    reads it in the browser, sends it in the request body, and strips it
    from the address bar. A missing or wrong token is refused, and the
    endpoint seals itself once the instance is claimed, as it always did.
  - **The claiming user becomes an admin of the org, not the instance
    operator.** A managed instance has no operator at all: the settings an
    operator would hold belong to the platform that runs it.
  - **A managed instance with no token configured refuses every claim**,
    and says so loudly in the startup log. An instance nobody can claim is
    a support call; an instance anybody can claim is somebody else's data.
  - **The token can be given an expiry**,
    `SLUICIO_BOOTSTRAP_TOKEN_EXPIRES_AT` (RFC 3339). Past it, a claim is
    refused exactly as a wrong token is. Whoever issues the token owns its
    lifetime - a new setup link is a new token - so this is a backstop for
    a link that was sent and never used, not the mechanism. A value that
    cannot be parsed stops a managed instance from starting rather than
    being read as "no expiry": a typo must not leave a token live for
    ever, and an instance that looks healthy while nobody can claim it is
    worse than one that says so on the way up.

On a managed instance these belong to the deployment rather than to
anyone signed in, and writing them answers 409:

  - the SMTP transport (`SLUICIO_SMTP_*`); the stored settings are ignored
    while managed mode is on, and email channels choose recipients rather
    than a server
  - retention, which follows the licence up to its limits and the built-in
    defaults without one; only issuing a licence changes it
  - the ingest URL (`SLUICIO_INGEST_URL`), the app URL (`SLUICIO_APP_URL`)
    and the environment name (`SLUICIO_ENVIRONMENT`)

`SLUICIO_ENVIRONMENT` is not specific to managed mode: set on any install,
it makes the environment label authoritative and read-only in the UI, the
same way `SLUICIO_INGEST_URL` already does for ingest.

**Requiring two-factor stays the org's.** It is an operator setting on a
self-hosted install and an org admin's on a managed one, because there is
no operator there and whether your own people must use 2FA is not the
platform's call. It keeps its Enterprise entitlement check either way.

Switching managed mode **off** again hands the instance back. The claiming
user becomes its operator on the next start - found by org role rather
than by the seeded address, which the claim replaced with their own - and
it is logged. Settings that managed mode locked are ignored while it is
on, never deleted, so they come back as they were.

Unset (or `false`), none of this applies and the install behaves exactly
as described above.
